...
- **Player count:** keep 1-4 in Config, queues, admission rosters and Studio direct entry. All required tasks must work solo; the shared bench supports cooperative handoffs and one seated operator.
...
- **Repair and stock:** the prototype has a generated parts room, three shelves, restock crate, authentic carried Tools, and one shared repair chair/order. ShiftService owns reservations and seat/work validation; Workshop owns physical Seat/Tool instances. Collect inside the room, deposit at the bench, sit to diagnose/fit/secure/install/test. Secure requires the next contact index, current order, and real seating; three contacts precede installation. Unused carried parts recover on death/disconnect/tool loss; deposited parts survive cancellation. Multi-order/workbench allocation, detailed disassembly, and authored hand animations remain future work.
...
- Physical Create/Leave requests require the current per-visit `EntryId`. Never allow delayed requests to alter a later visit. Generic Leave/Ready/Start/Cancel/Kick reject physical queue members. `LClient/QueueRequests` owns pending operations, bounded unanswered requests, timeout feedback, and stale-reply ownership; server snapshots remain authoritative. Deploy it as a sibling ModuleScript of QueueController and QueueView. Optional authored `Queue.Status` displays queued status and notifications; QueueView creates/removes a small fallback label when absent. Preserve existing control geometry and prevent ImageButton text updates from revealing hidden setup ancestors.
...
- `LClient/QueueBillboardMotion` adds brief local scale/alpha effects for world-sign phase changes, host/count/mode changes, and final countdown seconds 5-1. QueueController owns its lifecycle and uses the existing quarter-second update. Preserve authored geometry/corners/aspect constraints and UIScale baselines; cancel obsolete tweens before starting new ones, guard late completion callbacks, restore original alpha/scale on cleanup, and destroy only owned helpers. Never tween server-owned text, colors, visibility, or membership. First replication and ordinary gathering seconds do not pulse. Existing ReducedFlashes or per-Queue DisableQueueMotion suppresses effects.
- `LServer/QueueBillboard` renders the owner's updated `UI.BillboardGui` children: `GamemodeIcon`, `Gamemode`, `Title`, `Players`, and `GreenStatus`, plus Background. Empty means `SHIFT AVAILABLE / READY`; occupied titles use the current host's shift. Show actual count/capacity and the server departure deadline. Setup/final countdown/preparation use amber; ready/gathering/dispatch use green. Match an existing Background/direct board UIStroke without touching icon strokes. Preserve authored geometry/fonts; fit Title/GreenStatus using single-line text scaling. Legacy MapIcon/PartyLeader/Time remain fallbacks. Optional Queue attributes StoryIcon/EndlessIcon override the authored image; capture each replacement image instance's own fallback before applying an override.
...
- Lobby queue size-animation rule: preserve the current authored scaled `Size` and existing UIScale baseline. Only tween to 90% or 110% and back to 100%; never accumulate pulses, convert authored Size to offsets, or compound parent/child scale animations beyond 10%. Record this restriction for future queue UI work.

- Prototype UI uses black content panels at `BackgroundTransparency = 0.5`. Keep only the compact header/objective during ordinary play. SHIFT INFO exposes ticket/team/directive details; modals and local puzzle/timeclock interactions suppress the ordinary HUD. Preserve urgent feedback and a touch-accessible close/stand control. All generated prototype BillboardGuis use `UDim2.fromScale(4.8, 1.1)` (world studs) and `MaxDistance = 35`; this does not alter authored Lobby queue billboards.
