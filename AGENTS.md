...
- Game normal play uses `GClient/Interactions/FirstPersonCamera`: LockFirstPerson with a transparent Modal button for keyboard/mouse cursor release and right-click look. Enable Modal only for keyboard/mouse preferred input. Preserve native touch/controller camera controls and repair/timeclock Scriptable ownership; restore prior player/input settings on teardown. Lobby camera is unchanged.
